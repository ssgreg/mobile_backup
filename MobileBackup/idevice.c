...
{
	if (mode & CRYPTO_LOCK)
		cmn_mutex_lock(&mutex_buf[n]);
	else
		cmn_mutex_unlock(&mutex_buf[n]);
}
...
BOOL WINAPI DllMain(HINSTANCE hModule, DWORD dwReason, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		cmn_thread_once(&init_once,	internal_idevice_init);
		break;
	case DLL_PROCESS_DETACH:
		cmn_thread_once(&deinit_once, internal_idevice_deinit);
		break;
	default:
		break;
	}
